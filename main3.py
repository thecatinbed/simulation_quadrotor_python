...
def get_trajectory(type,t):
    ...
    if type :
        if t <= 10:
            ...
            position = (r * math.cos(2 * math.pi / 30 * (t-20)) - r, r * math.sin(2 * math.pi / 30 * (t-20)) , 10)
            position = (0, 0, 1)
            print("ok 3")
        else:
...
def main():
    ...
    horizonal_w0 = 10 / 1.6
    horizonal_wc = horizonal_w0 / 6#3.3
    horizonal_b0 = 1
    x_des_list = []
    ...
    while t_now < 60:
        #psi_des = math.sin(math.pi / 2  * t_now)
        #psi_des = math.pi / 12
        #psi_des,_ = psi_controller.trackingdifferentialtor.get_signal(psi_des)
        x_des = 0#3 * math.cos(2 * math.pi / 30 * t_now) - 3
        y_des = 0#3 * math.sin(2 * math.pi / 30 * t_now)
        h_des = 1
        x_des, y_des, h_des = get_trajectory(1, t_now)
        # 跟踪微分器柔化启动信号
        ...
        matrix_b_inverse = np.multiply(np.array([[math.cos(psi_v), math.sin(psi_v)],[math.sin(psi_v), -math.cos(psi_v)]]), -horizonal_b0/(QPara.g))
        print("matrix_b_inverse:",matrix_b_inverse)
        U = np.dot(matrix_b_inverse, np.array([[U1],[U2]]))
        print("U:",U)
        theta_des, phi_des = U[0], U[1]
        ...
        u3 = theta_controller.get_control_quantity_u_3d(theta_des)
        u2 = phi_controller.get_control_quantity_u_3d(phi_des)
        ...
        print("h_hat:",h_hat)
        #print("x_hat:",x_hat)
        #print("x_real:",quadrotors_model.measurement[0])

        y_list.append(quadrotors_model.measurement)  # 记录当前模型的输出
        y_hat_list.append(h_hat)  # 记录预测的输出

        t.append(t_now)
        t_now += QPara.dt  # 增加时间步长
        ctrl_buffer = np.vstack((ctrl_buffer, np.array([x_v,y_v,h_des,phi_des,theta_des,0])))

    plt.plot(t, [measurement[8] for measurement in y_list], label="psi_real")
    plt.plot(t, [y[2] for y in y_hat_list], label="psi_predict")
    #plt.plot(t, psi_des_list, label="psi_des")
    plt.grid(True)
    plt.legend()
    #plt.show()  # 图形可视化
    t.append(t_now)
    #print(len(t),len(ctrl_buffer), len(quadrotors_model.measurement_buffer))
...
def main2():
    ...
    controller = ADRC_controller.ADRC_Controller(QPara.dt,[25*2,25*25,0],5/3,math.sqrt(5))
    t_now = 0
    rdes = 1
    kp = 10
    y_list = [0]
...
def main3():
    ...
    controller = ADRC_controller.ADRC_Controller(QPara.dt, [w0*3, 3*w0*w0 ,w0*w0*w0], b0, wc)
    t_now = 0
    rdes = 1
    kp = 1
    y_list = []
